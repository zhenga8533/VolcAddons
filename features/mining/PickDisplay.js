...
const drillOverlay = new Overlay(
  "pickDisplay",
  data.PDL,
  "movePick",
  drillExample
...
registerWhen(
  register("step", () => {
    ...
    if (
      lore.find((line) => line.startsWith("§5§o§8Breaking Power")) === undefined
    )
      return;
    ...
    if (abilities.length === 0) {
      ...
      if (ability !== undefined)
        drillMessage += `\n  ${
          ...
          ability.substring(
            ability.indexOf(" "),
            ability.indexOf("  §e§lRIGHT CLICK")
          )
        } ${DARK_GRAY}[${RED}?${DARK_GRAY}]`;
    } else {
      abilities.forEach((key) => {
        const cd = cooldowns[key];
        drillMessage += `\n  ${GOLD + key + WHITE} ${DARK_GRAY}[${
          (cd > 0 ? RED : GREEN) + cd + DARK_GRAY
        }]`;
      });
    }
