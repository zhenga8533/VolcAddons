  ...
  register("spawnParticle", (particle, type) => {
    ...
    switch (type.toString()) {
      case "FOOTSTEP":
        ...
        if (
          lastBurrows.find(
            (lastBurrow) => lastBurrow[0] === x && lastBurrow[1] === z
          ) === undefined
        ) {
          lastBurrows.shift();
        ...
        if (closest[1] > 3 && World.getBlockAt(x, y, x).type?.getName()) {
          ...
          if (Settings.burrowDetect === 2 || Settings.burrowDetect === 4)
            playSound(AMOGUS, 100);
          if (Settings.burrowDetect === 3 || Settings.burrowDetect === 4)
            ChatLib.chat(
              `${
                LOGO + WHITE
              }Burrow Detected at ${GRAY}x: ${x}, y: ${y}, z: ${z}!`
            );
        }
        break;
      case "CRIT_MAGIC":
        if (closest[1] < 3)
          waypoints[waypoints.indexOf(closest[0])][0] = `§aStart`;
        break;
      case "CRIT":
        if (closest[1] < 3)
          waypoints[waypoints.indexOf(closest[0])][0] = `§cMob`;
        break;
    }
  }),
  () =>
...
registerWhen(
  register("chat", () => {
    const closest = burrows.getClosest([
      "Player",
      Player.getX(),
      Player.getY(),
      Player.getZ(),
    ...
    if (closest !== undefined)
      Client.scheduleTask(2, () =>
        waypoints.splice(waypoints.indexOf(closest[0]), 1)
      );
