...
const updateChocolate = register("tick", () => {
    ...
    if (chocoData) {
        ...
        cf.production = parseFloat(chocoData.getLore().find(line => line.endsWith("§8per second"))?.removeFormatting()?.replace(/,/g, "") ?? 0);
...
registerWhen(register("step", () => {
    ...
    const noTower = cf.production * (cf.multiplier - (towerData.activeTime > 0 ? towerData.bonus : 0)) / cf.multiplier;
