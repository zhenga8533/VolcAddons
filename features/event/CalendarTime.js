...
const tooltip = register("preItemRender", (_, __, slot) => {
  ...
  if (item.getName().startsWith("§aDay") && !item.getName().endsWith("]")) {
    ...
    const diff =
      container[0] === "Early" ? -1 : container[0] === "Late" ? 1 : 0;
    const month =
      3 *
        (["Spring", "Summer", "Autumn", "Winter"].indexOf(
          container[0 + Math.abs(diff)].replace(/,/g, "")
        ) +
          1) +
      diff -
      2;
    ...
    const until =
      month * 37_200 +
      day * 1_200 -
      ((new Date().getTime() / 1_000 - 107_704) % 446_400);
    const start = new Date(Date.now() + until * 1_000);
    const date = start.toLocaleDateString();
    const time = start.toLocaleTimeString();

    item.setName(
      `§aDay ${day + 1} §7[${YELLOW + date}, ${time
    ...
    for (let i = 0; i < lore.length + 1; i++) {
      if (
        lore[i]?.startsWith("§7Starts in:") &&
        !lore[i + 1]?.startsWith("§7Start Date")
      ) {
        startDate =
          unformatTime(lore[i].removeFormatting()) * 1_000 + Date.now();
        ...
        lore.splice(
          i + 1,
          0,
          `§7Start Date: ${YELLOW + date}, ${time.substring(
            0,
            time.length - 3
          )}`
        );
      } else if (
        lore[i]?.startsWith("§7Event lasts for") &&
        !lore[i + 1]?.startsWith("§7End Date")
      ) {
        const endDate =
          unformatTime(lore[i].removeFormatting()) * 1_000 + startDate;
        ...
        lore.splice(
          i + 1,
          0,
          `§7End Date: ${YELLOW + date}, ${time.substring(0, time.length - 3)}`
        );
      }
    }
  } else if (
    ...
    for (let i = 0; i < lore.length + 1; i++) {
      if (
        lore[i]?.startsWith("§7Starting in:") &&
        !lore[i + 1]?.startsWith("§7Start Date")
      ) {
        const startDate =
          unformatTime(lore[i].removeFormatting()) * 1_000 + Date.now();
...
registerWhen(
  register("guiOpened", () => {
    Client.scheduleTask(2, () => {
      ...
      if (
        split[0] === "Calendar" ||
        split[name.length - 2] === "Year" ||
        name === "SkyBlock Menu"
      )
        tooltip.register();
      else if (
        name.startsWith("Refine (Slot #") ||
        name.startsWith("Item Casting (Slot #")
      )
        forge.register();
      else return;
      close.register();
