...
const skillOverlay = new Overlay(
  "skillTracker",
  data.AL,
  "moveSkills",
  skillExample
...
const trackSkills = register("guiOpened", () => {
  Client.scheduleTask(3, () => {
    ...
    items.forEach((item) => {
      ...
      const maxIndex = lore.findIndex(
        (line) => line === "§5§o§7§8Max Skill level reached!"
      );
      const progressIndex = lore.findIndex((line) =>
        line.startsWith("§5§o§7Progress")
      );
      const xp =
        parseFloat(
          maxIndex !== -1
            ? lore[maxIndex + 1].split("§6")[1].replace(/,/g, "")
            : lore[progressIndex + 1]
                .split("§e")[1]
                .split("§6")[0]
                .replace(/,/g, "")
    ...
    if (Settings.skillTracker !== 0)
      setTitle(
        `${GREEN}Skills tracked!`,
        "Now begin the grind.",
        10,
        50,
        10,
...
register("command", () => {
  ...
  setTitle(
    `${GREEN}Successfully reset skills!`,
    "Please open skills menu to retrack.",
    10,
    50,
    10,
    81
...
registerWhen(
  register("actionBar", (health, gain, type, amount, next, mana) => {
    if (!skillsTracked) {
      setTitle(
        `${RED}Skills not tracked!`,
        "Please open skills menu to track.",
        0,
        50,
        10,
        79
...
registerWhen(
  register("actionBar", (health, gain, type, percent, mana) => {
    ...
    skill.now =
      xpTable[skill.level + 1] -
      (xpTable[skill.level + 1] - xpTable[skill.level]) * (1 - percent);
    skill.since = 0;
...
registerWhen(
  register("step", () => {
    ...
    // Set HUD
    const timeDisplay =
      skill.since < Settings.skillTracker * 60
...
${DARK_AQUA + BOLD}Level Up: `;
    ...
    if (skill.level !== 60) {
      ...
      if (neededXP > 0)
        skillMessage += `${WHITE + formatTime((neededXP / rate) * 3600)}`;
