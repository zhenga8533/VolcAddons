...
const coinageOverlay = new Overlay(
  "kuudraProfitTracker",
  data.ZL,
  "moveKPT",
  coinageExample,
...
function updateProfitTracker(openedChest) {
  if (openedChest) {
    ...
    data.kuudraSession.average =
      data.kuudraSession.profit / data.kuudraSession.chests;
  } else {
    kuudraSession.time++;
  ...
  const profitView =
    Settings.kuudraProfitTracker === 1 ? data.kuudraSession : kuudraSession;
  coinageOverlay.setMessage(
    `${DARK_RED + BOLD}Profit: ${
      WHITE + formatNumber(profitView.profit.toFixed(0))
    } ¢
${DARK_RED + BOLD}Chests: ${WHITE + commafy(profitView.chests)} chests
${DARK_RED + BOLD}Average: ${
      WHITE + formatNumber(profitView.average.toFixed(0))
    } ¢/chest
${DARK_RED + BOLD}Time: ${WHITE + formatTime(profitView.time)}
${DARK_RED + BOLD}Rate: ${
      WHITE + formatNumber(profitView.rate.toFixed(0))
    } ¢/hr`
  );
}
      ...
      secondary =
        bazaar[
          secondary
            ...
            .getString("id")
        ]?.[0] || getItemValue(secondary);
      let essence = container.getStackInSlot(14).getNBT().toObject().tag
        .display.Name;
      essence =
        parseInt(essence.slice(essence.indexOf("x") + 1)) *
        bazaar.ESSENCE_CRIMSON[0];
      const teeth = Settings.maxChili
        ? ((bazaar.ENCHANTMENT_TABASCO_3[0] - 64 * bazaar.CHILI_PEPPER[1]) /
            6) *
          Math.ceil(tier / 2)
        : 0;
      const cost =
        KEY_COST[tier - 1][0] +
        KEY_COST[tier - 1][1] *
          Math.min(bazaar.ENCHANTED_RED_SAND[1], bazaar.ENCHANTED_MYCELIUM[1]);
      const value = primary + secondary + essence + teeth;
      chestProfit = value - cost;

      const profitMessage =
        chestProfit >= 0
          ? `${GREEN}+${commafy(chestProfit)}`
          : `${RED}-${commafy(chestProfit)}`;
