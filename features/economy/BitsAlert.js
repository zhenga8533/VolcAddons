...
registerWhen(
  register("step", () => {
    const bits = Scoreboard.getLines().find((line) =>
      line.getName().startsWith("Bits:")
    );
    ...
    if (amount === last) {
      ...
      if (Settings.bitsAlert === 2 || Settings.bitsAlert === 3)
        ChatLib.chat(`${LOGO + DARK_AQUA + BOLD}NO MO BITS!`);
    }
    last = amount;
  }).setDelay(2700),
  ...
  register("guiOpened", () => {
    Client.scheduleTask(1, () => {
      ...
      if (container.getName() === "SkyBlock Menu") {
        ...
        if (
          !cookie[4].endsWith("Not active!") &&
          cookie[5] !== "§5§o§7Bits Available: §b0"
        )
          return;
        ...
        const bits = !container
          .getStackInSlot(11)
          .getLore()[7]
          .startsWith("§5§o§7Bits Available: §b0");
        const active = !(
          container.getStackInSlot(13).getLore()[20] ===
          "§5§o§7§cYou do not currently have a"
        );
