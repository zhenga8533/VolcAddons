  ...
  request({
    ...
    .then((response) => {
      ...
      response.auctions.forEach((auction) => {
        ...
        if (!bin) return;
        const item_data = new NBTTagCompound(
          decode(item_bytes).func_150305_b(0)
        )
          .getCompoundTag("tag")
      ...
      if (page + 1 < response.totalPages) findAttributes(page + 1, command);
        ...
        ChatLib.chat(
          `${DARK_GRAY}Attribute values saved, use '/refreshAttr' to refresh auction data!`
        );
      }
    })
...
register("command", () => {
  ...
  .setName("refreshAttr", true)
  .setAliases("refreshAttributes", "refreshAttribute");
...
export function getAttributes(args) {
  ...
  if (validCategories.has(category)) {
    // Attribute
    const item = args[1].includes("shard")
      ? "ATTRIBUTE_SHARD"
      : category.toUpperCase();
    const tier =
      args[2] !== undefined && !isNaN(args[2]) ? parseInt(args[2]) : 1;
    const attributes = auction[item]?.attributes || {};

    ChatLib.chat(
      `${
        LOGO + DARK_AQUA + BOLD + convertToTitleCase(item)
      } Attribute Prices (t${tier})`
    );
    ...
    if (attributeBin === undefined) {
      ...
      ChatLib.chat(
        `${
          LOGO + RED
        }Please input as: ${WHITE}/va attribute lbin [item] [attribute] *[min tier] *[amount]`
      );
      return;
    }
    ...
    const amount = isNaN(args[5])
      ? Math.min(10, bin.length)
      : Math.min(parseInt(args[5]), bin.length);
    ...
    new Message(
      `\n${
        LOGO + DARK_AQUA + BOLD
      }Top ${amount} t${min}+ ${attribute} ${piece}s:`
    )
    ...
    for (let i = 0; i < amount; i++) {
      new Message(
        `${i + 1}. `,
        new TextComponent(`${AQUA + bin[i][0]}`)
    ...
    const combo =
      category < piece ? `${category} ${piece}` : `${piece} ${category}`;
    ...
    KUUDRA_PIECES.forEach((piece) => {
      ...
      ARMOR_TYPES.forEach((armor) => {
        const price =
          auction[`${piece}_${armor}`]?.attribute_combos?.[combo] ?? 0;
        if (price < 10_000_000) worthless.push(chatID);
        new Message(
          `${DARK_GRAY}- ${AQUA + convertToTitleCase(armor)}: ${
            WHITE + formatNumber(price)
          }`
        )
          .setChatLineId(chatID)
          .chat();
    ...
    EQUIPMENT_TYPES.forEach((equip) => {
      ...
      if (price < 10_000_000) worthless.push(chatID);
      new Message(
        `${DARK_GRAY}- ${AQUA + convertToTitleCase(equip)}: ${
          WHITE + formatNumber(price)
        }`
      )
        .setChatLineId(chatID)
    ...
    new Message(
      new TextComponent(
        `${LOGO + GRAY}Click here to remove pieces worth less than 10m!`
      )
...
${
  LOGO + RED
}Please input as: ${WHITE}/va attribute ${GRAY}<${WHITE}shard, [armor], [equipment]${GRAY}> ${WHITE}[tier]
${
  LOGO + RED
}To check combo price, please input as: ${WHITE}/va attribute [attribute_1] [attribute_2]
${
  LOGO + RED
}To fetch lbin attributes, please input as: ${WHITE}/va attribute lbin [item] [attribute] *[min tier] *[amount]
${
  LOGO + RED
}To fetch piece upgrades, please input as: ${WHITE}/va attribute upgrade *[attribute] *[min tier] *[amount]
${
  DARK_GRAY + ITALIC
}Please note that values with more than one word need to be seperated by an underscore. (* means optional)`
    );
  }
}
