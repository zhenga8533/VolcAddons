...
if (!FileLib.exists("VolcAddons", "data"))
  new java.io.File("config/ChatTriggers/modules/VolcAddons/Data").mkdir();
if (!FileLib.exists("VolcAddons", "data/contract.txt"))
  FileLib.write(
    "VolcAddons",
    "data/contract.txt",
    FileLib.read("VolcAddons", "assets/contract.txt")
  );

// Utility Variable Control
const CHANGED_SETTINGS = new Set([
  "itemPrice",
  "bossAlert",
  "miniAlert",
  "vanqCounter",
...
for (const key in Settings)
  if (CHANGED_SETTINGS.has(key) && typeof Settings[key] !== "number")
    Settings[key] = 0;
...
const once = register("worldLoad", () => {
  ...
  delay(() => {
    ...
    if (version != data.version) {
      data.version = JSON.parse(
        FileLib.read("VolcAddons", "metadata.json")
      ).version;
      ChatLib.chat(
